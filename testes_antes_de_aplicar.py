...
def extrair_roi_corretamente(frame):
    ...
    return roi_cortada, (x, y), (x + w, y + h)
...
def contornos(frame):
    ...
    if largest_rect is not None: # Se uma area maior for encontrada, desenha
...
while True:
    ...
    if not ret:
        break
    frame_roi, (x, y), (x2, y2) = extrair_roi_corretamente(frame) # Corta area de interesse
    success, frame = contornos(frame)
    # frame = process_frame(frame)
    cv2.rectangle(frame, (x, y), (x2, y2), (255, 0, 0), 2) # desenha
